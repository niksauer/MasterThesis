...
\ref{itm:survey-task-transmit-measurements} can be explained by the fact that the research group would like to analyze measurements as early as possible. More so, they see the potential to expand this survey with the ability to give households a preview of the transactions and revenue or savings that could be achieved if the household were to actively participate in the \ac{EBP} (comp.~\cite{stoy2019broker}\todo{add page number}). This rules out any solutions in which the measurements are stored on device and only processed when the survey ends, i.e. after the devices have been returned. Instead, measurements shall be transferred to a centralized data store on an hourly schedule, at minimum (\textit{report cycle}).
...
\begin{itemize}
  ...
  \item Which of your \acsp{PVS} is connected to which of your generation meters?\todo{implicit}
\end{itemize}
